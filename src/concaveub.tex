...
In this section, we show that the corner injectivity theorem \ref{thm:injective} provides us an upper bound of the area of a monotone sofa $S$ which is concave respect to $\xx$.
...
$\frac{1}{2} \int_0^{\pi / 2} h(t)^2 + h(t)h''(t) + g(t)^2 + g(t)g''(t) dt$
...
Note that here, we are using the injectivity theorem \ref{thm:injective} to bound the area of the niche from below by the area determined by $\xx$.
...
\begin{equation}
\label{eq:areaub}
\frac{1}{2}\int_0^{\pi/2} h(t)h''(t)+g(t)g''(t)-h(t)g'(t)+g(t)h'(t) dt
\end{equation}
